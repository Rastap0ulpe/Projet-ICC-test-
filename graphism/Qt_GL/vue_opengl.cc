    ...
    for(size_t i(0);i<a;++i){
        for(size_t j(0);j<b;++j){
            for(size_t k(0);k<c;++k){
              ...
              if(a_dessiner.systeme_nuage(i,j,k)){
                ...
                std::vector<double> v(a_dessiner.get_systeme_vitesse(i,j,k));

                prog.setAttributeValue(CouleurId, 1.0, 0.0, 0.0);
                if(a_dessiner.get_systeme_norme(i,j,k) > 25){
                vec.x=v[0]/a_dessiner.get_systeme_norme(i,j,k);
                vec.y=v[1]/a_dessiner.get_systeme_norme(i,j,k);
                vec.z=v[2]/a_dessiner.get_systeme_norme(i,j,k);
